...
$ ./bad_default_value
thread 3973430 panic: reached unreachable code
/home/ci/work/zig-bootstrap/out/host/lib/zig/std/debug.zig:421:14: 0x1025eb9 in assert (std.zig)
    if (!ok) unreachable; // assertion failure
             ^
/home/ci/work/zig-bootstrap/zig/doc/langref/bad_default_value.zig:8:15: 0x11db20a in categorize (bad_default_value.zig)
        assert(t.maximum >= t.minimum);
              ^
/home/ci/work/zig-bootstrap/zig/doc/langref/bad_default_value.zig:19:42: 0x11d9cbe in main (bad_default_value.zig)
    const category = threshold.categorize(0.90);
                                         ^
/home/ci/work/zig-bootstrap/out/host/lib/zig/std/start.zig:685:59: 0x11da29c in callMain (std.zig)
...
- An [integer](09-integers.md#Integers) field uses exactly as many bits as its
  bit width. For example, a `u5` will use 5 bits of
  the backing integer.
- A [bool](#Primitive-Types) field uses exactly 1 bit.
- An [enum](17-enum.md#enum) field uses exactly the bit width of its integer tag type.
...
on the backing integer. These operations are not [atomic](36-atomics.md#Atomics),
so beware using field access syntax when combined with memory-mapped
input-output (MMIO). Instead of field access on [volatile](#volatile) [Pointers](14-pointers.md#Pointers),
...
anonymous: struct_name.main__struct_32464
...
the struct literal will directly instantiate the [result location](33-result-location-semantics.md#Result-Location-Semantics),
with no copy:
...
The struct type can be inferred. Here the [result location](33-result-location-semantics.md#Result-Location-Semantics)
...
[identifiers](05-identifiers.md#Identifiers).
