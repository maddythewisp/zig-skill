...
**Current stable:** Zig 0.16.0 (as of this documentation)

**Recommended versions for production:**
- **0.16.0** - Current stable, recommended for new projects (async I/O reintroduced)
- **0.15.2** - Previous stable, still widely used
- **0.14.1** - Older stable with good ecosystem support

**Legacy versions:**
- **0.11.0-0.13.0** - Early modern Zig, major breaking changes from 0.10
- **0.9.1-0.10.1** - Last versions with old async/await (removed in 0.11)
...
**Key Features:**
- **MAJOR:** Async I/O reintroduced via new `std.Io` interface
- New `io.async()` / `future.await(io)` API (replaces old async/await keywords)
- No function coloring — same code works in sync and async contexts
- Runtime-polymorphic I/O implementations (Blocking, ThreadPool, EventLoop)
- Explicit `io.concurrent()` for true parallelism requirements
...
**Async Migration (from 0.10.x old async):**
```zig
// Old (0.10.x - removed in 0.11)
var frame = async saveFile(data, "a.txt");
try await frame;

// New (0.16.0+)
var fut = io.async(saveFile, .{io, data, "a.txt"});
try fut.await(io);
```

### 0.15.x (Previous Stable)
...
- No async/await (removed in 0.11, reintroduced in 0.16)
...
**The `std.Io` interface replaces the old async/await keywords (removed in 0.11):**
...
#### 2. Async vs Concurrent

**Critical distinction new in 0.16:**

```zig
// io.async() - operations CAN proceed out-of-order (sequential awaiting is fine)
var fut = io.async(work, .{io});

// io.concurrent() - operations MUST run simultaneously (errors if parallelism unavailable)
var fut = try io.concurrent(work, .{io});
```

Use `io.async()` by default. Only use `io.concurrent()` for producer-consumer patterns or when simultaneous execution is required.
...
0.16 supports multiple I/O backends:
- **Blocking I/O** — standard syscalls, zero overhead
- **Thread Pool** (`std.Io.Threaded`) — multiplexes across OS threads
- **Event Loop** — io_uring/kqueue with green threads (planned)
...
**Synchronous 0.15 code works in 0.16 without changes. Async is opt-in.**
...
**Migration:** In 0.11-0.15, use `std.Thread` for concurrency. In 0.16+, use the new `std.Io` async interface.

### Async I/O Reintroduction (0.16+)

| Feature | 0.11-0.15 | 0.16+ | Notes |
|---------|-----------|-------|-------|
| Async capability | Not available | `io.async()` | New API, not old keywords |
| Concurrent execution | `std.Thread` | `io.concurrent()` | Explicit parallelism |
